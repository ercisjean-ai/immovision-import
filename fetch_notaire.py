...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Collecte des annonces Notaire.be/immo et les convertit vers le format interne."
    )
    parser.add_argument(
        "--search-url",
        help="URL de recherche Notaire.be a collecter",
    )
    parser.add_argument(
        "--html-file",
    ...
    parser.add_argument(
        "--output",
        default=str(Path("sample_data") / "notaire_latest.jsonl"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Nombre maximum de pages Notaire a parcourir en mode live",
    )
    ...
    try:
        if args.html_file:
            items = collect_notaire_listings(html_file=args.html_file, timeout=args.timeout)
            collection_mode = "fixture"
        else:
            collection_result = collect_notaire_listing_result(
    ...
    except NotaireFetchError as exc:
        raise SystemExit(f"Collecte Notaire.be impossible: {exc}") from exc

    output_path = write_listings_jsonl(args.output, items)

    print(f"Collecte Notaire.be ({collection_mode}): {len(items)} annonces")
    if collection_result is not None:
        print(format_notaire_coverage_summary(collection_result))
